...
\begin{figure}
\label{fig:domain}\includegraphics[scale=1.3]{fig1}
...
subdomain are available through the NIS data portal \citep{Gori:etal:data:2016}.
...
covariance \citep{Lind:etal:2011}. These models are
...
simple \citep{Rue:Held:2005}. However, in situations
...
the $Q$ matrix defined previously as follows based
...
is available on Github, including pre- and post-processing code, at\\\url{https://github.com/PalEON-Project/composition}. 
...
the $j$th held-out tree in the $i$th cell is of taxon $p$. $\hat{\theta}_{ip}=Y_{ip}/n_{i}$
...
\vspace{2mm}
\hspace{2mm}\includegraphics[scale=0.16]{legendRaw}\hspace{4mm}\includegraphics[scale=0.16]{legendProp}\hspace{4mm}\includegraphics[scale=0.16]{legendSD}
...
\begin{align}
\alpha_{p} & \sim\mbox{N}\bigg(\Big(A+Q_{p}\Big)^{-1}A\bar{w}_{p},\Big(A+Q_{p}\Big)^{-1}\bigg). \nonumber
\end{align}
